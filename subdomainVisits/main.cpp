...
public:
    map<string,int> m;
    void countDomains(string &s){
        int pos = s.find(" ");
        string domain = s.substr(pos+1);//取域名
        int num = stoi(s.substr(0,pos));//取次数
        pos = -1;
        do{
            domain = domain.substr(pos+1);
            if(m.find(domain) == m.end())//如果map里没有,那就插入
                m.insert(map<string, int>::value_type (domain,num));
            else
                m[domain] += num;//有就加上本次输入的次数
            pos = domain.find(".");//查找'.',如果有点，那就还可以拆分
        }while(pos >= 0);
    }
    vector<string> subdomainVisits(vector<string>& cpdomains) {
        vector<string> res;
        ...
        for(auto &obj : m){//转化结果
            res.push_back(to_string(obj.second)+" "+obj.first);
        }
        return res;
    }
};
